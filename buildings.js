...
class BuildingManager {
    ...
    async loadBuildingsFromMasterCSV() {
        try {
            ...
            const totalBuildings = Object.values(buildingData).reduce((sum, cat) => sum + cat.length, 0);
            console.log('Loaded', totalBuildings, 'buildings from master CSV');
            return buildingData;
        } catch (error) {
...
window.loadBuildingsFromMasterCSV = async function() {
    console.log('🏗️ Loading all buildings from master CSV...');
    ...
    if (result) {
        ...
        console.log('✅ Successfully loaded', totalBuildings, 'buildings from master CSV');
        // Refresh the game UI
        if (window.game && window.game.populateBuildingCategories) {
            window.game.populateBuildingCategories();
        }
...
window.clearHousing = function() {
    ...
    console.log('✅ Housing cleared');
};

window.resetBuildings = function() {
    console.log('🔄 Resetting all buildings and loading from master CSV...');
    window.buildingManager.clearAllBuildings();
    // Force reload from CSV
    setTimeout(async () => {
        await window.loadBuildingsFromMasterCSV();
    }, 100);
    console.log('✅ Buildings reset, master CSV loading...');
};

window.forceReloadCSV = async function() {
    console.log('🔄 Force reloading all buildings from master CSV...');
